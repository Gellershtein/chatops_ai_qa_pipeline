...
def save_context_to_minio(ctx: dict):
    ...
    upload_json(config.minio_bucket, get_context_minio_path(run_id), serializable_ctx)
...
def load_context_from_minio(run_id: str) -> dict:
    ...
    """
    loaded_ctx = download_json(config.minio_bucket, get_context_minio_path(run_id))
