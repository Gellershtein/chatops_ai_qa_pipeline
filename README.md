...
1.  **Telegram Бот**: Основной интерфейс для взаимодействия с пользователем. Бот принимает файлы с требованиями, пайплайн управлять его выполнением и получать артефакты.    
2.  **AI Модель (codegemma:7b (local model))**: Используется для генерации сценариев, тест-кейсов, автотестов, а также для анализа кода и создания отчетов.    
...
-   `pipeline/`:    
    ...
    -   `steps/`: Каждый файл в этой директории представляет собой отдельный шаг AI QA пайплайна (например, `generate_scenarios.py`, `ai_code_review.py`). 
   -   `llm/`:    
	    -   `gemini_client.py`: Клиент для взаимодействия с API Gemini.    
	    -   `prompts/`: Модули с промптами для AI для каждого конкретного шага.    
-   `storage/`: Содержит клиент для работы с объектным хранилищем Minio (`minio_client.py`). **Примечание: `minio_setup.py` также присутствует, но его использование может потребовать проверки.**-   `models/`: Определяет структуры данных, используемые в проекте.    
...
### Инструкция по установке    
 ...
 ```bash git clone <URL вашего репозитория>  cd chatops_ai_qa_pipeline_v3 ``` 2.  **Создайте файл `.env`:**    
